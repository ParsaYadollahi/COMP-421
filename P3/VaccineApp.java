    ...
    public static void assignSlot(Connection con, Statement statement, int sqlCode, String sqlState) {
      ...
      try {
        ...
        if (numDoesesNeeded - numDosesTaken == 0 && vnameTaken != ""){
          System.out.println("The resident with Health insurance number : " + hinsurnumDetails +  " does not need any more doses!");
          statement.close () ;
          con.close () ;
          return;
        }
        ...
        System.out.println(checkNurseAssignmentsQuery);
         ...
         if(nurseAssignmentsRC.next()) {
            if (vname.equals("")) {
              ...
              System.out.println(updateSQL);
              statement.executeUpdate(updateSQL);
              System.out.println("Resident with Health Insurance Number " + hinsurnumDetails + " has successfully been assigned to inputed slot.");
            } else {
